...
def settings() -> dict:
    ...
    param = [[sg.Text("Bandwidth in MHz: "), sg.Combo([1.4, 3, 5, 10, 15, 20],default_value=1.4, size=(4,1), key="bandwidth")], 
            [sg.Text("Nombre de communications"), sg.Input(size=(4,1), key="com", default_text=2)], 
            [sg.Text("Temps Max entre 2 com (def : 1s)"), sg.Input(size=(4,1), key="interval", default_text=2)], 
            #[sg.Text("Ecart-type"), sg.Input(size=(4,1))],
            #[sg.Text("tailles des communication"), sg.Input(size=(4,1))],
            [sg.Text("taille moyenne des comm (def : 10kbts)"), sg.Input(size=(4,1), key="av_size", default_text=2)],
            #[sg.Text("ecart-Type"), sg.Input(size=(4,1))],
